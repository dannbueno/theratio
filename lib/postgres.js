...
// Función para actualizar una sesión de atleta directamente (raw)
export async function updateAthleteSessionRaw(sessionData) {
  if (!pool) {
    console.warn('No hay conexión a la base de datos disponible.');
    return null;
  }
  ...
  try {
    const { id, access_token, refresh_token, expires_at, ...otherFields } = sessionData;
    
    // Construir consulta dinámica
    let updateQuery = 'UPDATE strava_tokens SET updated_at = NOW()';
    const queryParams = [];
    let paramCounter = 1;
    
    // Añadir campos a actualizar si están presentes
    if (access_token) {
      updateQuery += `, access_token = $${paramCounter}`;
      queryParams.push(access_token);
      paramCounter++;
    }
    
    if (refresh_token) {
      updateQuery += `, refresh_token = $${paramCounter}`;
      queryParams.push(refresh_token);
      paramCounter++;
    }
    
    if (expires_at) {
      updateQuery += `, expires_at = $${paramCounter}`;
      queryParams.push(expires_at);
      paramCounter++;
    }
    
    // Añadir condición WHERE y RETURNING
    updateQuery += ` WHERE athlete_id = $${paramCounter} RETURNING *`;
    queryParams.push(id);
    ...
    console.error('Error actualizando sesión de atleta:', error);
    return null;
  }
}

// Función para obtener todas las sesiones de un atleta específico
export async function getSessionsByAthlete(athleteId) {
  if (!pool) {
    console.warn('No hay conexión a la base de datos disponible.');
    return [];
  }
  ...
  try {
    ...
    console.error('Error obteniendo sesiones por atleta:', error);
    return [];
  }
}
